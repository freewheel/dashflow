...
function onError(err) {
  process.stderr.write(
    "Dashflow quit due to the error:\n" + chalk.red(err.message) + "\n"
  );
...
async function startOneOff(config, commands) {
  ...
  if (unknownCommands.length > 0) {
    onError(new Error(`unknown commands ${commands.join(", ")}`));
  }
...
async function start(configs, port = null, commands = []) {
  ...
  if (commands.length > 0) {
    ...
    // daemon mode
    const app = startDaemon(config, onError);
    // serve HTTP and websocket
    const finalPort = port || (await findPort());

    serve(app, finalPort, onError);
